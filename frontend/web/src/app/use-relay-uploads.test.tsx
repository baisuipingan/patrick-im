  ...
  beforeEach(() => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.endsWith('/api/relay/upload-request')) {
          return new Response(
            JSON.stringify({
              fileId: 'file-1',
        ...
        if (url.endsWith('/api/relay/abort') || url.endsWith('/api/relay/discard')) {
          return new Response(null, { status: 200 });
        }

        if (url.endsWith('/api/relay/complete')) {
          return new Response(
            JSON.stringify({
              fileId: 'file-1',
              objectKey: 'obj/file-1',
        ...
        if (url.endsWith('/api/relay/upload-part')) {
          return new Response(null, { status: 200 });
        }
